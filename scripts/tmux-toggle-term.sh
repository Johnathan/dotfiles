...
SHELL_WINDOW_ID="$(tmux list-windows -t "$SESSION" -F '#{window_id} #{window_name}' \
...
if [[ -n "$SHELL_WINDOW_ID" ]]; then
  ...
  tmux join-pane -f -v -s "$SHELL_WINDOW_ID" -l 15% -t "$SESSION:$WINDOW"
elif [[ "$PANE_COUNT" -ge 3 ]]; then
  # Find the bottom pane by position and break it out
  BOTTOM_PANE="$(tmux list-panes -t "$SESSION:$WINDOW" -F '#{pane_index} #{pane_top}' \
    | sort -k2 -n | tail -1 | cut -d' ' -f1)"
  tmux break-pane -d -s "$SESSION:$WINDOW.$BOTTOM_PANE" -n "$SHELL_WINDOW_NAME"
else
  # No shell pane exists — create one at the bottom
  DIR="$(tmux display-message -p '#{pane_current_path}')"
  tmux split-window -f -v -l 15% -c "$DIR"
fi
